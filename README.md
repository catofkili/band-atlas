...
**989 支乐队与音乐人 / 2314 条关系**，平均每位 4.7 条；其中东亚 428 位，地区未知 54 位。
...
边缘邻居、搜索结果和地图都不会把用户带进超冷门节点。当前 989 个节点会收紧到
551 个，分享链接指定的当前焦点例外保留。ListenBrainz 只代表其开放数据
...
但覆盖不均。当前 671 位有流派、345 位有代表曲；354 位简介仍标记为机器事实摘要，
它们不会进入高质量随机首屏池。随机入口在 447 位非模板、关系不少于三条的候选中，
