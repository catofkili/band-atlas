...
**990 支乐队与音乐人 / 2318 条关系**，平均每位 4.7 条；其中东亚 429 位，地区未知 54 位。
骨架由 MusicBrainz 爬来，现代日本增量同时允许组合与个人音乐人，简介取自维基百科；27 支由人工完整打底，
...
ListenBrainz 历史总收听 1000 次为界；开启时，随机换一支、焦点卡片、关系线、
边缘邻居、搜索结果和地图都不会把用户带进超冷门节点。当前 990 个节点会收紧到
552 个，分享链接指定的当前焦点例外保留。ListenBrainz 只代表其开放数据
...
但覆盖不均。当前 672 位有流派、346 位有代表曲；344 位简介仍标记为机器事实摘要，
它们不会进入高质量随机首屏池。随机入口在 456 位非模板、关系不少于三条的候选中，
